...
def train_next_batch(M, data_indices, d_batch_size, transform):
    ...
    if ADD_NOISE:
        batch_x = batch_x + 0.005*np.random.normal(size=batch_x.shape)
        #batch_y = batch_y + 0.01*np.random.normal(size=batch_y.shape)
        
    batch_y = np.min([batch_y, np.ones_like(batch_y)], 0)
...
def decode(filename, ID, PREDICTION, lammps_dir='./Model/results/LAMMPS/'):
    # Specify resolution
    #resolution = 64
