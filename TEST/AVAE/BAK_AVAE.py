...
class AVAE(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        #self.adv_loss_tolerance = 0.1     # [blurred]
        ##self.adv_loss_tolerance = 0.25   # [pixelated]
        #self.adv_loss_tolerance = 0.175   # [pixelated]
        self.adv_loss_tolerance = 0.33
        self.l2_weight = 1.0
    ...
    def decoder(self, z, reuse=None):
        with tf.variable_scope("decoder", reuse=reuse):
            ...
            #net = transpose_inception_v3(net, 96, stride=2, training=self.is_training, name='tincept_0', reuse=reuse)
            new_res = 2*self.latent_res
            net = tf.image.resize_images(net, [new_res, new_res], method=tf.image.ResizeMethod.BILINEAR, align_corners=False)
            net = inception_v3(net, 64, stride=1, training=self.is_training, name='incept_1', reuse=reuse)

            # Res/8
            #net = transpose_inception_v3(net, 64, stride=2, training=self.is_training, name='tincept_1', reuse=reuse)
            new_res = 4*self.latent_res
            net = tf.image.resize_images(net, [new_res, new_res], method=tf.image.ResizeMethod.BILINEAR, align_corners=False)
            net = inception_v3(net, 64, stride=1, training=self.is_training, name='incept_2', reuse=reuse)
            ...
            new_res = 8*self.latent_res
            net = tf.image.resize_images(net, [new_res, new_res], method=tf.image.ResizeMethod.BILINEAR, align_corners=False)
            net = inception_v3(net, 32, stride=1, training=self.is_training, name='incept_3', reuse=reuse)
    ...
    #    C1 = (K1*L)**2
    #    C2 = (K2*L)**2
    #    m_x = tf.reduce_mean(x,axis=[1,2,3])
    #    m_y = tf.reduce_mean(y,axis=[1,2,3])
    #    return index
