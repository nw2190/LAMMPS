...
{\rd{performed on the basis of the partial differential equations}},
but the partial derivatives are not present on the surface of the crack,
{\rd{so it is not enough to get accurate results}}.
%%
%Therefore, i
...
Peridynamics is the non-local extension of classical continuum mechanics \cite{Ref15}. The model structure of peridynamics is the same as an MD model {\rd{and in LAMMPS, SI units (m, kg, sec etc.) can be used for simulations}}.
...
%%
{\rd{Thus, as mentioned above, peridynamics is a non-local extension of the classical continuum mechanics.}}
%%
...
$\zeta$ + $\eta$ represents the current relative position vector between the particles. The pairwise bond force density $\omega$ {\rd{should have}} the following properties: 
%eq2
\begin{equation}
...
\noindent Here $k$ is the bulk modulus of the material and $G_0$ is the work required {\rd{to crack the all bonds}} per unit fracture area. As seen from the equations above, the spring constant and the critical bond stretch are composed of the bulk modulus of the material $k$ and chosen horizon boundary $\delta$. Finally, in the case of brittle materials such as glass, {\rd{related to bond stretch}}, {\rd{is defined as follows}}:
...
\end{equation}
 ...
 $s_{00}$ and $\alpha$ {\rd{are constants}} and $\alpha$ is generally used about 1/4. 
...
%%
Each particle $i$ {\rd{has volume fraction}} $V_i$=1.25 \times 10$^{-10}$m$^3$, and the density of the disk material is $\rho$=2200kg/m$^3$.
%%
...
%%
$c$ is the spring constant for peridynamic bonds, the horizon {\rd{is a cutoff distance}},
%%
and the constants $s_{00}$ and $\alpha$ {\rd{are used as}} critical bond stretch parameters.
%%
...
%%
{\rd{SI units are to be used}} and the boundary is non-periodic and {\rd{shrink-wrapped [clarification?]}}.
%%
The particle volume {\rd{is used in a}} simple cubic lattice with lattice constant 0.001m.
%%
A cylinder disk (target) with radius 0.037m and thickness 0.0025 m is initialized.
%%
The initial velocity of all particles is set to zero, and the hitting location, velocity, and radius of the indenter {\rd{should be saved to do damage prediction for the next steps after the MD simulation, so we set up to save the parameters information in indenter.txt}}.
%%
...
%%
{\rd{Finally, we declare a dump command to obtain the crack pattern image, and 2000 time-steps of running simulation to get the data are used, and we set it to 2000 time-steps in dump command as well.}}
%%
...
%%
{\rd{The symmetry solution represents the right side in the figure because of the perfect lattice is used, and the perfectly spherical indenter impacted the geometric center of the target.}}
%%
In this study, the use of asymmetric crack patterns could provide better data for real world applications
{\rd{but focused primarily on crack patterns with symmetry, However, as the position of the hitting point change, various crack patterns can be confirmed, so that the data for machine learning is sufficient.}}
%4
...
%%
 {\rd{The labeled data set which is called the training pair is $\rho$=($\mathcal{X}_i$,$\omega_i$).}}
%%
 This pair represents the damage set with corresponding labels.
%%
 % In this work, 70\% of the input data and 10\% of the input data set are used for the training set and the test set respectively and 30\% of the training data, is used for the validation set.
  In this work, 70\% of the data set examples are used for training, 30\% of the data is used for validation, and 10\% is used for testing the model {\rd{[should add up to 100\%]}}.
%%
 The labeled training set is used in the supervised learning algorithm which {\rd{is chosen and designed for making a prediction model.}}
%%
...
%%
 Lastly, in order to evaluate the prediction model, a separate set of data, referred to as the test set, is used to assess the accuracy of the model.
%%
...
%%
{\rd{[Needs a transition.]}}
%%
An inverse problem is a mathematical way to predict the parameters through the measured and observed data.
%%
{\rd{ In this work, the crack pattern on a disk by an indenter was supposed to be analyzed depending on the different value of the parameters of the indenter through the way of the inverse problem.}}
%%
...
%%
{\rd{ Prior to considering the results,}} focusing on the input data using LAMMPS, {\rd{we can classify into 8 modes changing the 4 variables}} (the radius of the indenter, the velocity of the indenter, the angle of the indenter, and the hitting location of the disk) and obtained 7,200 training examples and 1,200 test examples {\rd{[what about validation?]}}.
%%
...
\label{subsubsec3}
Deep learning is a subset of AI and machine learning {\rd{that can bring state-of-the-art technology and accuracy}} in problems such as object detection, text generation, and image recognition.
%%
{\rd{Using existing human knowledge in annotated data}}, various cognitive and inferential tasks can be performed \cite{Ref23,Ref24,Ref25}.
%%
...
%%
{\rd{This work accounts for eight categories from mode 1 to mode 8.}}
...
%%
{\rd{In this study, the final layer should be 1x1 spatial resolution and since the size of the filter should be classified as 8 modes, it should be unconditionally 8.}}
%%%
