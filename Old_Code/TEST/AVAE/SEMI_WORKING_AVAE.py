...
class AVAE(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir):
        ...
        self.latent_res = 4
        self.latent_chans = 16

        # Loss function weights
        self.loss_tolerance = 0.33
        self.adv_loss_tolerance = 20.0
        self.kl_loss_tolerance = 20.0
        self.l2_weight = 1.0
    ...
    def encoder(self, x):
        with tf.variable_scope("encoder", reuse=None):
            ...
            if self.use_onehot:
                ...
                # res = 16
                net = conv2d_layer(net, 16, 3, stride=1, add_bias=False, training=self.is_training, name='en_conv_2')
    ...
    def decoder(self, z, reuse=None):
        with tf.variable_scope("decoder", reuse=reuse):
            ...
            net = transpose_conv2d_layer(net, 32, kernel_size=2, stride=2, training=self.is_training, name='g_tconv_1', reuse=reuse)
            net = transpose_conv2d_layer(net, 16, kernel_size=3, stride=2, training=self.is_training, name='g_tconv_2', reuse=reuse)
            net = transpose_conv2d_layer(net, self.c_dim, kernel_size=4, stride=2, activation=None,
                                         add_bias=False, regularize=False, training=self.is_training, name='g_tconv_4', reuse=reuse)

            #out = tf.clip_by_value(net,0.0,1.0)
    ...
    def build_model(self):
        ...
        self.L2_loss = tf.reduce_mean(tf.reduce_mean(tf.pow(self.pred-self.outputs, 2), axis=[1,2,3]))
        ...
        a_vars = [var for var in t_vars if not 'd_' in var.name]
        e_vars = [var for var in t_vars if 'en_' in var.name]
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_opt = tf.train.AdamOptimizer(learning_rate=self.d_weight*self.learning_rate, epsilon=1e-06) \
                                 .minimize(self.d_loss, var_list=d_vars)
            self.g_opt = tf.train.AdamOptimizer(learning_rate=self.g_weight*self.learning_rate, epsilon=1e-06) \
                                 .minimize(self.g_loss, var_list=g_vars)
            self.g2_opt = tf.train.AdamOptimizer(learning_rate=self.g2_weight*self.learning_rate, epsilon=1e-06) \
                                  .minimize(self.loss, var_list=g_vars)
            self.opt = tf.train.AdamOptimizer(learning_rate=self.e_weight*self.learning_rate, epsilon=1e-06) \
                               .minimize(self.loss, var_list=e_vars)
