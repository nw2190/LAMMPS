...
[y_min, y_max] = [-0.0025 - y_tolerance, 0.000 + y_tolerance]
...
## Calculate Neighbor Damag
def neighbor_damage(avg_damage,inds):
    [i,k,j] = inds
...
def encode(ID):
    ...
    avg_damage = avg_damage*template
        ...
        for j in range(0,y_count):
            for k in range(0,z_count):
                if bin_counts[i,k,j] == 0:
                    if template[i,k,j] == 1:
                        if USE_NEIGHBORS:
                            nb_d = neighbor_damage(avg_damage,[i,k,j])
                            if nb_d == 0:
                                avg_damage[i,k,j] = 1.0
                            else:
                                avg_damage[i,k,j] = nb_d
                        else:
                            avg_damage[i,k,j] = 1.0
    """
...
def decode(filename, ID, PREDICTION, lammps_dir):
    # Specify resolution
    #resolution = 64
