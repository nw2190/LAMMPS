    ...
    def encoder(self, x):
        with tf.variable_scope("encoder", reuse=None):
            ...
            if self.use_onehot:
                ...
                net = conv2d_layer(net, 32, 5, stride=1, add_bias=False, training=self.is_training, name='d_conv_1')
                net = layers.max_pooling2d(net, 3, 2, padding='same', data_format='channels_last')
                
                # res = 16
                #net = conv2d_layer(net, 16, 3, stride=1, add_bias=False, training=self.is_training, name='d_conv_2')
                net = inception_v3(net, 16, stride=1, add_bias=False, training=self.is_training, name='d_incept_1')
                net = layers.max_pooling2d(net, 3, 2, padding='same', data_format='channels_last')
                
                # res = 8
                #net = conv2d_layer(net, 8, 2, stride=1, training=is_training, name='d_conv_3')
                net = inception_v3(net, 8, stride=1, add_bias=False, training=self.is_training, name='d_incept_2')
                net = layers.max_pooling2d(net, 2, 2, padding='same', data_format='channels_last')

                # res = 4
                #net = tf.reshape(net,[-1,int(16*self.res//8*self.res//8)])
                net = tf.reshape(net, [self.batch_size, -1])
                net = lrelu(linear(net, self.z_dim, scope='d_fc1'))
            ...
            gaussian_params = dense_layer(net, 2*self.z_dim, training=self.is_training,
                                          regularize=False, activation = None, name='d_dense_enc')
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                for transformation in self.transformations:
                    ...
                    if np.mod(counter, self.validation_step) == 0:
                        ...
                        self.reg_weight = np.min([self.reg_weight, 0.5*l2_loss/reg_loss])
