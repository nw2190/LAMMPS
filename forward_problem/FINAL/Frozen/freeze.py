...
def freeze_graph(model_folder,CV):
    # Specify checkpoint to freeze
    checkpoint_dir = model_folder + "Checkpoints/model_" + str(CV) + "/"
    checkpoint = tf.train.get_checkpoint_state(checkpoint_dir)
    input_checkpoint = checkpoint.model_checkpoint_path
    
    # Specify filename for frozen graph
    output_graph = model_folder + "frozen_model_" + str(CV) + ".pb"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--CV", default=1, type=int, help="Cross Validation ID to export")
    args = parser.parse_args()
    
    freeze_graph(args.model_folder, args.CV)
